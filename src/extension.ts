...
interface LineAnalysis {
  ...
  chordMidis?: number[];
  chordNotes?: string[];
  bassCandidateMidi?: number;
  bassCandidateNote?: string;
  dedupedBass?: boolean;
  newlyTriggeredBass?: boolean;
  triggersSynth?: boolean;
}
...
let triggerDecoration: vscode.TextEditorDecorationType;
...
let splitBelowDecoration: vscode.TextEditorDecorationType;
let splitAboveDecoration: vscode.TextEditorDecorationType;
let synthOffsetCommandDecoration: vscode.TextEditorDecorationType;
...
class AscoStateProvider implements vscode.TreeDataProvider<StateItem> {
  private _onDidChangeTreeData = new vscode.EventEmitter<StateItem | undefined | void>();
  readonly onDidChangeTreeData = this._onDidChangeTreeData.event;

  private items: StateItem[] = [];

  refresh(items: StateItem[]) {
    this.items = items;
  ...
  getChildren(): Thenable<StateItem[]> {
...
function isEnabled(): boolean {
  return vscode.workspace.getConfiguration("ascoState").get<boolean>("enabled", true);
}

function trackedKeywords(): string[] {
  return vscode.workspace
    .getConfiguration("ascoState")
    .get<string[]>("trackKeywords", DEFAULT_KEYWORDS);
}

function allowedExtensions(): string[] {
  return vscode.workspace
    .getConfiguration("ascoState")
    .get<string[]>("fileExtensions", [".asco", ".asco.txt"]);
}
...
function parseStateCommand(line: string, keywords: string[]): { key: string; value: string } | undefined {
  ...
  const key = parts[0];
  if (!keywords.includes(key)) return undefined;

  return { key, value: parts.slice(1).join(" ") };
}

function parseSynthRange(value: string): [number, number] | undefined {
...
function pitchClassToSemitone(pc: string): number | undefined {
...
function noteNameToMidi(name: string): number | undefined {
  ...
  const pc = (m[1].toUpperCase() + m[2]) as string;
...
function parseSplitValue(rawValue: string | undefined): number | undefined {
  if (!rawValue) return undefined;
  const value = Number(rawValue.trim().split(/\s+/)[0]);
  return Number.isNaN(value) ? undefined : value;
}

function freshMidisForLine(line: LineAnalysis): number[] {
  const values: number[] = [];
  if (line.noteMidi !== undefined) values.push(line.noteMidi);
  if (line.chordMidis && line.chordMidis.length > 0) values.push(...line.chordMidis);
  return values;
}

function describeSynthOffset(rawValue: string): string {
  const value = Number(rawValue.trim().split(/\s+/)[0]);
  if (Number.isNaN(value)) return rawValue;
  if (value === 0) return "offset 0";
...
function parseNoteLine(line: string): { noteName: string; midi: number } | undefined {
  ...
  const noteName = m[1];
  if (noteName.startsWith("-")) return undefined; // held/tied note
...
function parseChordLine(line: string): { noteNames: string[]; midis: number[] } | undefined {
  return parseChordLikeLine(line, "CHORD");
}

function parseChordLikeLine(line: string, keyword: "CHORD" | "TRILL"): { noteNames: string[]; midis: number[] } | undefined {
  const trimmed = stripComment(line).trim();
  ...
  if (rawNotes.length === 0) return undefined;
...
function bassCandidateFromRange(
  state: ScoreState,
  midiValues: number[],
  noteNames?: string[]
): { midi: number; noteName?: string } | undefined {
...
function analyseDocument(doc: vscode.TextDocument): {
  lines: LineAnalysis[];
  ...
  const stateChanges: StateChangeItem[] = [];
  ...
  for (let i = 0; i < doc.lineCount; i++) {
    ...
    let noteMidi: number | undefined;
    let noteName: string | undefined;
    let chordMidis: number[] | undefined;
    let chordNotes: string[] | undefined;
    let bassCandidateMidi: number | undefined;
    let bassCandidateNote: string | undefined;
  ...
  let lastPlayedBass: number | undefined;

  for (const line of lines) {
    if (line.bassCandidateMidi === undefined) continue;
...
function rangesForLines(doc: vscode.TextDocument, lineNumbers: number[]): vscode.Range[] {
...
function refreshEditor(editor: vscode.TextEditor | undefined, provider: AscoStateProvider) {
  ...
  const splitBelowLines: number[] = [];
  const splitAboveLines: number[] = [];
  ...
  editor.setDecorations(
    synthOffsetCommandDecoration,
    synthOffsetChanges.map(change => {
      const line = editor.document.lineAt(change.line);
      return {
        range: new vscode.Range(change.line, 0, change.line, line.text.length),
        renderOptions: {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("ascoState.refresh", () => {
      refreshEditor(vscode.window.activeTextEditor, provider);
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand("revealLine", async (line: number) => {
  ...
  context.subscriptions.push(
    vscode.window.onDidChangeActiveTextEditor(editor => refreshEditor(editor, provider)),
    vscode.window.onDidChangeTextEditorSelection(e => refreshEditor(e.textEditor, provider)),
